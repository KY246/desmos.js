...
Calc.colors.DARKBLUE="rgb(0,0,150)";
...
Calc.colors.LIGHTYELLOW="rgb(255,255,150)";
Calc.colors.GOLD="rgb(255,200,0)";
Calc.colors.BAIGE="rgb(255,255,200)";
Calc.colors.LIGHTBLUE="rgb(150,150,255)";
function down(e){
  if(e.key=="@"){
    try{
      var color=prompt("Enter your color code here (ie. rgb(0,0,0), #000000)\nWARNING typing an invalid color code, or an nonunique color code will stop desmos from working. Proceed with caution:");
      if(color!=null){
        Calc.colors[Math.max(Calc.colors.length+2,30)]=color;
      }
    }catch(error){
      alert("ERROR: "+error);
    }
